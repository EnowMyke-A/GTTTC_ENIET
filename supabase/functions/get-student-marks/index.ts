...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!courseId || !termId || !academicYearId) {
      return new Response(
        JSON.stringify({ error: 'Missing required parameters: courseId, termId, academicYearId' }),
        { 
          status: 400, 
    ...
    // Filter by department if course has specific department
    if (courseData.department_id) {
      eligibleStudents = eligibleStudents.filter(
        (cs: any) => cs.students.department_id === courseData.department_id
      );
    }
    ...
    if (marksError) {
      ...
      return new Response(
        JSON.stringify({ error: 'Error fetching marks' }),
        { 
          status: 500, 
          headers: { ...corsHeaders, 'Content-Type': 'application/json' } 
        }
    ...
    const studentMarksData = eligibleStudents.map((cs: any) => {
      ...
      const existingMark = marksData?.find((m: any) => m.student_id === student.id);
