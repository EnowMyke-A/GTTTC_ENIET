...
const Analytics = () => {
  ...
  const [classMasterDepartment, setClassMasterDepartment] =
    useState<string>("");
  ...
  const fetchInitialData = async () => {
    try {
      ...
      const hasAnnual = allTerms.some(
        (term) => term.label?.toLowerCase() === "annual"
      );
      if (!hasAnnual) {
        allTerms.push({
          id: "annual",
      ...
      // Set department and level based on user type AFTER all data is loaded
      if (isClassMasterLocal) {
        ...
        console.log(
          "Set class master filters:",
          classMasterDeptId,
          classMasterLvlId
        ); // Debug log
      } else if (userRole === "admin") {
        // For admins, default to "all"
    ...
    try {
      ...
      data.data.summaryDocument.statistics.forEach((stat: any) => {
        ...
        if (subjectMap.has(subjectName)) {
          ...
          existing.totalScoreSum +=
            (stat.classAverage || 0) * (stat.enrollment?.total || 0);
      ...
      const combinedSubjects = Array.from(subjectMap.values()).map(
        (subject) => {
          // Calculate weighted average
          subject.average =
            subject.totalEnrollmentSum > 0
              ? parseFloat(
                  (subject.totalScoreSum / subject.totalEnrollmentSum).toFixed(
                    2
                  )
                )
              : 0;
          ...
          subject.passedGte10.boys_percentage =
            subject.enrollment.boys > 0
              ? parseFloat(
                  (
                    (subject.passedGte10.boys / subject.enrollment.boys) *
                    100
                  ).toFixed(1)
                )
              : 0;
          subject.passedGte10.girls_percentage =
            subject.enrollment.girls > 0
              ? parseFloat(
                  (
                    (subject.passedGte10.girls / subject.enrollment.girls) *
                    100
                  ).toFixed(1)
                )
              : 0;
          subject.passedGte10.total_percentage =
            subject.enrollment.total > 0
              ? parseFloat(
                  (
                    (subject.passedGte10.total / subject.enrollment.total) *
                    100
                  ).toFixed(1)
                )
              : 0;
          ...
          const { totalScoreSum, totalEnrollmentSum, ...cleanSubject } =
            subject;
          return cleanSubject;
        }
      );
      ...
      const summaryData = {
        ...
        subjects: combinedSubjects.sort((a, b) => {
      ...
      saveAs(
        ...
        `${termLabel}_Summary_Statistics_${
      ...
      toast.error(
        "Error downloading Excel: " + (error.message || "Unknown error")
      );
    } finally {
      ...
      const printContent = `
        ...
        <body>
          <div class="header">
            ...
            <div class="info">
              Academic Year: ${
                analytics.summaryDocument?.header?.academicYear || "N/A"
              } | 
              Department: ${
                analytics.summaryDocument?.header?.department ||
                "All Departments"
              } | 
              ...
              <div class="stat-card">
                ...
                <div class="stat-item">
                  ...
                  <span class="stat-value">${
                    analytics.enrollmentStats?.enrolled?.total || 0
                  }</span>
                </div>
                <div class="stat-item">
                  <span>Boys:</span>
                  <span class="stat-value">${
                    analytics.enrollmentStats?.enrolled?.boys || 0
                  }</span>
                </div>
                <div class="stat-item">
                  <span>Girls:</span>
                  <span class="stat-value">${
                    analytics.enrollmentStats?.enrolled?.girls || 0
                  }</span>
                </div>
              </div>
              <div class="stat-card">
                ...
                <div class="stat-item">
                  <span>Students:</span>
                  <span class="stat-value">${
                    analytics.enrollmentStats?.satForExams?.total || 0
                  }</span>
                </div>
                <div class="stat-item">
                  <span>Percentage:</span>
                  <span class="stat-value">${
                    analytics.enrollmentStats?.satForExams?.percentage || 0
                  }%</span>
                </div>
              </div>
              <div class="stat-card">
                ...
                <div class="stat-item">
                  ...
                  <span class="stat-value highlight">${
                    analytics.passFailStats?.passed?.total || 0
                  }</span>
                </div>
                <div class="stat-item">
                  <span>Overall Pass Rate:</span>
                  <span class="stat-value highlight">${
                    analytics.passFailStats?.passed?.percentageTotal || 0
                  }%</span>
                </div>
              </div>
            </div>
            ...
            <div class="stats-grid">
              <div class="stat-card">
                ...
                <div class="stat-item">
                  <span>Passed:</span>
                  <span class="stat-value">${
                    analytics.passFailStats?.passed?.boys || 0
                  }</span>
                </div>
                <div class="stat-item">
                  <span>Failed:</span>
                  <span class="stat-value">${
                    analytics.passFailStats?.failed?.boys || 0
                  }</span>
                </div>
                <div class="stat-item">
                  <span>Pass Rate:</span>
                  <span class="stat-value highlight">${
                    analytics.passFailStats?.passed?.boys
                      ? (
                          (analytics.passFailStats.passed.boys /
                            (analytics.passFailStats.passed.boys +
                              analytics.passFailStats.failed.boys)) *
                          100
                        ).toFixed(1)
                      : 0
                  }%</span>
              ...
              <div class="stat-card">
                ...
                <div class="stat-item">
                  <span>Passed:</span>
                  <span class="stat-value">${
                    analytics.passFailStats?.passed?.girls || 0
                  }</span>
                </div>
                <div class="stat-item">
                  <span>Failed:</span>
                  <span class="stat-value">${
                    analytics.passFailStats?.failed?.girls || 0
                  }</span>
                </div>
                <div class="stat-item">
                  <span>Pass Rate:</span>
                  <span class="stat-value highlight">${
                    analytics.passFailStats?.passed?.girls
                      ? (
                          (analytics.passFailStats.passed.girls /
                            (analytics.passFailStats.passed.girls +
                              analytics.passFailStats.failed.girls)) *
                          100
                        ).toFixed(1)
                      : 0
                  }%</span>
          ...
          <div class="section">
            ...
            ${analytics.top3Students
              ?.map(
                (student, index) => `
              <div class="list-item">
                <span>${index + 1}. ${student.name} (${student.gender}, ${
                  student.department
                })</span>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 mt-6">
                {[1, 2].map((i) => (
                  <div
                    key={i}
                    className="h-64 rounded-md bg-muted-foreground/5 animate-pulse"
                  />
                ))}
              </div>
            </div>
    ...
    return (
      <div className="p-6 text-center">
        ...
        <p className="text-muted-foreground/80">
          Only administrators and class masters can access analytics.
        </p>
      </div>
    );
  }
          ...
          analytics.enrollmentStats?.satForExams?.percentage > 0 && (
            <div className="flex gap-3">
              <Button
                ...
                disabled={
                  downloadingPDF || !selectedAcademicYear || !selectedTerm
                }
              ...
              <Button
                ...
                disabled={
                  downloadingExcel || !selectedAcademicYear || !selectedTerm
                }
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
              <Label htmlFor="department">
                Department {isClassMaster && "(Locked)"}
              </Label>
              <Select
          ...
          <CardContent className="p-6">
            <div className="text-center py-12">
              ...
              <p className="text-muted-foreground/80 max-w-md mx-auto">
                Select your filters above to view comprehensive statistics and
                analytics for the academic performance.
              </p>
            </div>
          </CardContent>
        </Card>
            ...
            <div className="text-center space-y-4">
              ...
              <div className="space-y-2">
                ...
                <p className="text-sm text-muted-foreground/80 max-w-md mx-auto">
                  No students have written exams for the selected academic year
                  and term. Statistics will be available once exam scores are
                  entered.
                </p>
              </div>
            </div>
              ...
              <CardContent className="p-6">
                <div className="flex items-start justify-between">
                  <div className="flex flex-col gap-2">
                    <p className="text-sm font-bold text-muted-foreground/85">
                      Total Students
                    </p>
                    <p className="text-xl font-bold min-h-[2rem] flex items-center">
                      {analytics.enrollmentStats?.enrolled?.total || 0}
                    </p>
              ...
              <CardContent className="p-6">
                <div className="flex items-start justify-between">
                  <div className="flex flex-col gap-2">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {analytics.passFailStats?.passed?.total || 0} of{" "}
                      {analytics.enrollmentStats?.satForExams?.total || 0}{" "}
                      students
                    </p>
                  </div>
              ...
              <CardContent className="p-6">
                <div className="flex items-start justify-between">
                  <div className="flex flex-col gap-2">
                    ...
                    <p className="text-xl font-bold min-h-[2rem] flex items-center">
                      {analytics.summaryDocument?.statistics?.length || 0}
                    </p>
                    <p className="text-xs text-muted-foreground">
                  ...
                  <PieChart className="text-sm">
                    <Pie
                      ...
                      data={[
                        {
                          name: "Passed",
                          value: analytics.passFailStats?.passed?.total || 0,
                        },
                        {
                          name: "Failed",
                          value: analytics.passFailStats?.failed?.total || 0,
                        },
                      ]}
                      cx="50%"
                      cy="50%"
                ...
                <ResponsiveContainer width="100%" height={300}>
                  <BarChart
                    ...
                    data={[
                      {
                        category: "Passed",
                        boys: analytics.passFailStats?.passed?.boys || 0,
                        girls: analytics.passFailStats?.passed?.girls || 0,
                      },
                      {
                        category: "Failed",
                        boys: analytics.passFailStats?.failed?.boys || 0,
                        girls: analytics.passFailStats?.failed?.girls || 0,
                      },
                    ]}
                  >
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">
                            Level {course.level} • {course.enrollment}{" "}
                            student(s) • {course.passRate}% pass rate
                          </p>
                        </div>
                      </div>
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 bg-red-500 text-white rounded-full flex items-center justify-center text-xs font-bold">
                            {Math.abs(
                              analytics.summaryDocument?.statistics?.length -
                                ((analytics.summaryDocument?.statistics
                                  ?.length !== 2
                                  ? 2
                                  : 1) -
                                  index)
                      ...
                      >
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground">
                              {student.gender} • Level {student.level} •{" "}
                              {toTitleCase(student.department)}
                            </p>
                          </div>
                        </div>
                        <div className="text-right">
                    ...
                    (student: any, index: number) => (
                      ...
                      >
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 bg-red-500 text-white rounded-full flex items-center justify-center text-xs font-bold">
                            {Math.abs(
                              analytics.enrollmentStats?.satForExams?.total -
                                ((analytics.enrollmentStats?.satForExams
                                  ?.total !== 2
                                  ? 2
                                  : 1) -
                                  index)
                      ...
                      selectedDepartment === "all"
                        ? // Show department performance when "All Departments" is selected
                          analytics.departmentPerformance?.details
                            ...
                            .map((dept) => ({
                              subject:
                                dept.abbreviation?.length > 15
                                  ? dept.abbreviation.substring(0, 15) + "..."
                        ...
                        : // Show course averages when specific department is selected
                          (() => {
                            ...
                            analytics.summaryDocument?.statistics?.forEach(
                              (stat: any) => {
                                const subjectName = stat.subject;
                            ...
                            return Array.from(courseMap.values())
                              .map((course) => ({
                                subject:
                                  course.subject.length > 15
                                    ? course.subject.substring(0, 15) + "..."
                                    : course.subject,
                                average:
                      ...
                      <Legend
                        content={() => {
                          ...
                          const sortedDepts = [...departments].sort(
                            (a: any, b: any) =>
                              b.departmentScore - a.departmentScore
                          );
                            ...
                            >
                              {sortedDepts.map((dept: any, index: number) => (
                                ...
                                >
                                  ...
                                  <span>
                                    {dept.abbreviation || dept.name}:{" "}
                                    {dept.departmentScore.toFixed(1)}
                                  </span>
                                </div>
                              ))}
                            </div>
